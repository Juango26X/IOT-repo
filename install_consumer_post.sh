#!/bin/bash
# Amazon Linux 2023 - Consumer POST via Docker
...
sudo docker pull ${docker_user}/iot-worker-post:latest
sudo docker run -d --restart=always --name worker_post \
  -e DB_HOST="$${DB_HOST}" \
  -e DB_PORT=5432 \
  -e DB_NAME=orders_db \
  -e DB_USER=postgres \
  ...
  ${docker_user}/iot-worker-post:latest
